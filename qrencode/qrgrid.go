...
const (
	PENALTY_WEIGHT_N1 = 3
	PENALTY_WEIGHT_N2 = 3
	PENALTY_WEIGHT_N3 = 40
	PENALTY_WEIGHT_N4 = 10
)

func bestMaskPattern(bits *BitVector, version versionNumber, ecLevel ECLevel, grid *BitGrid) int {
	bestMaskPattern := -1
	bestPenalty := 0
	for maskPattern := 0; maskPattern < NUM_MASK_PATTERNS; maskPattern++ {
	...
	for y := 0; y < grid.Height(); y++ {
		for x := 0; x < grid.Width(); x++ {
			if x+6 < grid.Width() &&
				...
				!grid.Get(x+5, y) &&
				grid.Get(x+6, y) &&
				(isWhiteHorizontal(grid, y, x-4, x) || isWhiteHorizontal(grid, y, x+7, x+11)) {
				penalty++
			}
			if y+6 < grid.Height() &&
				grid.Get(x, y) &&
				...
				!grid.Get(x, y+5) &&
				grid.Get(x, y+6) &&
				(isWhiteVertical(grid, x, y-4, y) || isWhiteVertical(grid, x, y+7, y+11)) {
				penalty++
			}
		}
	}
	return penalty * PENALTY_WEIGHT_N3
}
...
func maskPenaltyRule4(grid *BitGrid) int {
	...
	fivePercentVariances := int(math.Abs(float64(dark*2-total)) * 10.0 / float64(total))
	return fivePercentVariances * PENALTY_WEIGHT_N4
}

func isWhiteHorizontal(grid *BitGrid, rowIndex int, from int, to int) bool {
	for i := from; i < to; i++ {
		if i >= 0 && i < grid.Width() && grid.Get(i, rowIndex) {
			return false
		}
	}
	return true
